...
class PartnerState(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if state:
            try:
                # Shop.objects.filter(user_id=request.user.id).update(state=distutils.util.strtobool(state))
                Shop.objects.filter(user_id=request.user.id).update(state=xdistutils.util.strtobool(state))

                # Shop.objects.filter(user_id=request.user.id).update(state=state_bool)
                return JsonResponse({'Status': True})
